...
export default async function HomePage({ params }: HomePageProps) {
  ...
  const projects = getProjects(loc).filter((p) => p.featured);
    ...
    <div className="flex flex-col">
      ...
      >
        <div className="max-w-[90%] mx-auto w-full">
          <HeroAnimation text={ui.home.heroText} />
        </div>
      </ParallaxBackground>
        ...
        <div className="max-w-[90%] mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {projects.map((project, idx) => (
              <ProjectCard key={project.slug} project={project} index={idx} learnMoreLabel={ui.cta.learnMore} locale={loc} />
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-3xl mx-auto flex flex-col items-center relative z-10">
          <Reveal direction="up" delay={0.1}>
            <h2 className="text-3xl md:text-5xl font-normal leading-tight mb-8 font-poppins">
              {ui.home.nickelHeading}
            </h2>
          </Reveal>
          <Reveal direction="up" delay={0.3}>
            <a
              href={`/${loc}/whatwedo/renewable-nickel-battery-materials`}
